...
def load_values():
    ...
    values = np.asarray([[float(row[name]) for name in columns] for row in rows],
                        dtype=np.float32)
...
class WindowDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        targets = self.values[index + LOOKBACK:index + LOOKBACK + HORIZON,
                              TARGET_INDICES]
...
class MultiStepLSTM(nn.Module):
    def __init__(self, input_features, num_targets):
        super().__init__()
        self.lstm = nn.LSTM(input_features, 64, num_layers=2, dropout=0.1,
                            batch_first=True)
...
def run_epoch(model, loader, loss_fn, optimizer=None):
    ...
    with context:
        for features, targets in loader:
            ...
            predictions = model(features); loss = loss_fn(predictions, targets)
            if training:
                optimizer.zero_grad(); loss.backward(); optimizer.step()
            total += loss.item() * len(features); count += len(features)
    return total / count
...
def main():
    ...
    loss_fn = nn.MSELoss(); optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
    best = float("inf")
    for epoch in range(1, EPOCHS + 1):
        train_mse = run_epoch(model, train_loader, loss_fn, optimizer)
        val_mse = run_epoch(model, val_loader, loss_fn)
        if val_mse < best:
            best = val_mse; torch.save(model.state_dict(), CHECKPOINT)
    ...
    with torch.no_grad():
        for features, labels in test_loader:
            predictions.append(model(features.to(DEVICE)).cpu().numpy()); targets.append(labels.numpy())
    ...
    mae = np.mean(np.abs(predictions - targets)); rmse = np.sqrt(np.mean((predictions - targets) ** 2))
