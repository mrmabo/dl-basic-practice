...
class MultiSeriesDataset(Dataset):
    ...
    def __getitem__(self, i):
        return self.data[i : i + LOOKBACK], self.data[i + LOOKBACK, 0:1]  # [L,3], [1]
...
class TransformerForecaster(nn.Module):
    def __init__(self, in_features=3, d_model=32, nhead=4, layers=2):
        ...
        layer = nn.TransformerEncoderLayer(
            d_model, nhead, dim_feedforward=64, dropout=0.1, batch_first=True
        )
...
def main():
    ...
    t = torch.arange(3600).float()
    raw = torch.stack(
        [
            torch.sin(t / 12) + 0.1 * torch.randn(3600, generator=g),
            torch.cos(t / 12),
            torch.sin(t / 50),
        ],
        dim=1,
    ...
    a, b = 2400, 3000
    mean, std = raw[:a].mean(0), raw[:a].std(0)
    z = (raw - mean) / std
    train = MultiSeriesDataset(z[:a])
    val = MultiSeriesDataset(z[a - LOOKBACK : b])
    test = MultiSeriesDataset(z[b - LOOKBACK :])
    ...
    model.load_state_dict(
        torch.load("best_transformer.pt", map_location=DEVICE, weights_only=True)
    )
    print(f"test_mse={eval_mse(model,test_loader,loss_fn):.5f}")
    x, y = next(iter(test_loader))
    pred = model(x[:5].to(DEVICE)).detach().cpu() * std[0] + mean[0]
    true = y[:5] * std[0] + mean[0]
    print("pred:", pred.squeeze().round(decimals=3).tolist())
    print("true:", true.squeeze().round(decimals=3).tolist())
