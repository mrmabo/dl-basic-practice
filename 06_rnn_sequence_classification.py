...
class TrendDataset(Dataset):
    ...
    def __init__(self, n=2400):
        ...
        base = torch.linspace(-1, 1, SEQ_LEN).repeat(n, 1)
        direction = self.y.float() * 2 - 1
        offset = torch.randn(n, 1, generator=g)
        noise = 0.25 * torch.randn(n, SEQ_LEN, generator=g)
...
def main():
    ...
    train, val, test = random_split(
        TrendDataset(), [1600, 400, 400], generator=torch.Generator().manual_seed(SEED)
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            f"epoch={epoch:02d} train_loss={total/len(train):.4f} val_loss={val_loss:.4f} val_acc={val_acc:.3f}"
        )
    model.load_state_dict(
        torch.load("best_rnn.pt", map_location=DEVICE, weights_only=True)
    )
    print("test:", evaluate(model, test_loader, loss_fn))
    x, y = next(iter(test_loader))
    pred = model(x[:8].to(DEVICE)).argmax(1).cpu()
    print("pred:", pred.tolist(), "true:", y[:8].tolist())
